  ...
  <div class="orders-page">
    <div class="container">
      ...
      <div class="filters">
        ...
        >
          Активные <span v-if="stats?.active" class="count">{{ stats.active }}</span>
        </button>
        <button
          :class="['filter-btn', { active: activeFilter === 'history' }]"
          @click="setFilter('history')"
        >
          История <span v-if="stats?.history" class="count">{{ stats.history }}</span>
        </button>
      </div>
      ...
      <div v-else-if="orders.length === 0" class="empty">
        ...
        <p>{{ activeFilter === 'active' ? 'У вас пока нет активных заказов' : 'История пуста' }}</p>
        <router-link v-if="activeFilter === 'active'" to="/catalog" class="go-catalog"
          ...
          <div class="order-card-inner">
            ...
            <div class="order-content">
              ...
              <div class="order-details">
                ...
                <div v-if="order.isDeleted" class="detail-row">
                  <span class="detail-label">Удалён:</span>
                  <span class="detail-value">{{ formatDate(order.deletedAt) }}</span>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    async loadOrders() {
      try {
        this.loading = true
        const data = await ordersAPI.getMyOrders(this.activeFilter)
        this.orders = data.orders || []
        this.stats = data.stats || { active: 0, history: 0 }
      } catch (error) {
    ...
    async deleteOrder(orderId) {
      ...
      try {
        await ordersAPI.deleteMyOrder(orderId)
        await this.loadOrders()
      } catch (error) {
    ...
    canDelete(order) {
      return this.activeFilter === 'active' && ['completed', 'cancelled'].includes(order.status)
    },
    statusLabel(status) {
      const map = {
        pending: 'Ожидает',
