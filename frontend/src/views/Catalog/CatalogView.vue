<template>
  <div class="catalog-view">
    <div class="container">
      ...
      <!-- Основная область: фильтры + сетка товаров -->
      <div class="catalog-content">
        ...
        <div class="catalog-products">
          ...
          <div v-else-if="productsResponse.products.length === 0" class="products-empty">
          ...
          <div v-else class="products-grid" :class="`products-grid--${viewMode}`">
            <ProductCard
...
const totalPages = computed(() => Math.ceil(productsResponse.total / productsResponse.limit))
...
const loadCatalog = async () => {
  try {
    ...
    if (route.params.categoryId) {
      currentCategory.value = categories.value.find(c => c.id === route.params.categoryId) || null
    }
    
    if (route.params.subcategoryId) {
      const subcategories = response.data.subcategories || {}
      currentSubcategory.value = subcategories[route.params.subcategoryId as string] || null
    }
  } catch (error) {
