        ...
        <div class="tree-container">
          ...
          >
            <div
              ...
              @click="selectNode('category', category)"
            >
              <button
                class="tree-toggle"
              ...
              <button class="tree-action" @click.stop="editCategory(category)">
                ✏️
              </button>
            </div>
            ...
            >
              <div
                v-for="brand in getBrandsForCategory(category.id)"
                :key="brand.id"
                class="tree-node"
              >
                <div
                  class="tree-item brand"
                  :class="{
                    selected:
                      selected?.type === 'brand' && selected?.id === brand.id,
                  }"
                  @click="selectNode('brand', brand)"
                >
                  <button
                    class="tree-toggle"
                  ...
                  >
                    <svg
                      v-if="expandedBrands.includes(brand.id)"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      stroke-width="2"
                  ...
                  <button class="tree-action" @click.stop="editBrand(brand)">
                    ✏️
                  </button>
                </div>
                ...
                >
                  <div
                    v-for="model in getModelsForBrand(brand.id)"
                    :key="model.id"
                    class="tree-node"
                  >
                    <div
                      class="tree-item model"
                      :class="{
                        selected:
                          selected?.type === 'model' &&
                          selected?.id === model.id,
                      }"
                      @click="selectNode('model', model)"
                    >
                  ...
                  <div
                    class="tree-item add-item"
                    @click="openModelModal(category.id, brand.id)"
                  >
                    <span class="tree-toggle-placeholder"></span>
      ...
      <div class="info-panel">
        ...
        <div v-if="selected?.type === 'category'" class="info-content">
          ...
          <div class="info-stats">
            <div class="stat-card">
              <span class="stat-value">{{
                getBrandCount(selected.item.id)
              }}</span>
              <span class="stat-label">Брендов</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{
                getModelCount(selected.item.id)
              }}</span>
              <span class="stat-label">Моделей</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{
                getProductCount(selected.item.id)
              }}</span>
          ...
          <div class="info-actions">
            <button class="btn-primary" @click="editCategory(selected.item)">
              Редактировать
            </button>
            <button class="btn-ghost" @click="openBrandModal(selected.item.id)">
              Добавить бренд
            </button>
          </div>
        </div>
        ...
        <div v-if="selected?.type === 'brand'" class="info-content">
          ...
          <div class="info-meta">
            <div class="meta-row">
              ...
              <span class="meta-value">{{
                getCategoryName(selected.item.categoryId)
              }}</span>
            </div>
          ...
          <div class="info-stats">
            <div class="stat-card">
              <span class="stat-value">{{
                getModelCountForBrand(selected.item.id)
              }}</span>
              <span class="stat-label">Моделей</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{
                getProductCountForBrand(selected.item.name)
              }}</span>
              <span class="stat-label">Товаров</span>
            </div>
          </div>
          ...
          <div class="info-actions">
            <button class="btn-primary" @click="editBrand(selected.item)">
              Редактировать
            </button>
            <button
              class="btn-ghost"
              @click="
                openModelModal(selected.item.categoryId, selected.item.id)
              "
            >
              Добавить модель
            </button>
        ...
        <div v-if="selected?.type === 'model'" class="info-content">
          ...
          <div class="info-meta">
            <div class="meta-row">
              ...
              <span class="meta-value">{{
                getBrandName(selected.item.id)
              }}</span>
            </div>
            <div class="meta-row">
              <span class="meta-label">Категория:</span>
              <span class="meta-value">{{
                getCategoryNameByModel(selected.item.id)
              }}</span>
            </div>
          </div>
          ...
          <div class="info-stats">
            <div class="stat-card">
              <span class="stat-value">{{
                getProductCountForModel(selected.item.id)
              }}</span>
              <span class="stat-label">Товаров</span>
            </div>
          </div>
          ...
          <div class="info-actions">
            <button class="btn-primary" @click="editModel(selected.item)">
              Редактировать
            </button>
            <button
              class="btn-primary"
              @click="openProductModal(selected.item)"
            >
              Добавить товар
            </button>
          </div>
...
const getBrandCount = (categoryId: string) =>
  brands.value.filter((b) => b.categoryId === categoryId).length;
...
const getProductCount = (categoryId: string) =>
  products.value.filter((p) => p.categoryId === categoryId).length;
const getModelCountForBrand = (brandId: string) =>
  models.value.filter((m) => m.brandId === brandId).length;
const getProductCountForBrand = (brandName: string) =>
  products.value.filter((p) => p.brand === brandName).length;
const getProductCountForModel = (modelId: string) =>
  products.value.filter((p) => p.modelId === modelId).length;
...
const openProductModal = (model: any) => {
  router.push(
    `/admin/products/new?modelId=${model.id}&categoryId=${
      model.categoryId || model.subcategoryId
    }`
  );
};
